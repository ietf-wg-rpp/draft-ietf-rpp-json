...
All message objects are generated by the server and are read-only from the perspective of the receiving client.

#### Base Message Object

The Base Message Object is an abstract base structure and MUST NOT appear on its own in a message; it is always extended by one of the message types defined below.

The following constraints cannot be expressed in JSON Schema and MUST be enforced by implementations:

- `objectId` MUST be a valid identifier for the object type specified in `objectType`.
...
```json
{
  "$defs": {
    "baseMessage": {
      "type": "object",
      "properties": {
        "objectType": { "type": "string", "readOnly": true, "enum": ["domain", "host", "contact"] },
    ...
    "reviewResponse": {
      "allOf": [
        { "$ref": "#/$defs/baseMessage" },
        {
          "type": "object",
          "properties": {
            "@type":               { "type": "string", "const": "reviewResponse", "readOnly": true },
            "approved":            { "type": "boolean", "readOnly": true },
            "clientTransactionId": { "type": "string", "readOnly": true },
            "serverTransactionId": { "type": "string", "readOnly": true },
            "completionDate":      { "type": "string", "format": "date-time", "readOnly": true },
            "message":             { "type": "string", "readOnly": true }
          },
          "required": ["@type", "approved", "serverTransactionId", "completionDate", "message"]
        }
      ]
    }
  }
    ...
    "transferRequest": {
      "allOf": [
        { "$ref": "#/$defs/baseMessage" },
        {
          "type": "object",
          "properties": {
            "@type":       { "type": "string", "const": "transferRequest", "readOnly": true },
            "status":      { "type": "string", "const": "pending", "readOnly": true },
            "requestDate": { "type": "string", "format": "date-time", "readOnly": true },
            "actionDate":  { "type": "string", "format": "date-time", "readOnly": true }
          },
          "required": ["@type", "status", "requestDate"]
        }
      ]
    }
  }
    ...
    "transferOutcome": {
      "allOf": [
        { "$ref": "#/$defs/baseMessage" },
        {
          "type": "object",
          "properties": {
            "@type": { "type": "string", "const": "transferOutcome", "readOnly": true },
            "trStatus": {
              "type": "string",
              "enum": ["clientApproved", "clientCancelled", "clientRejected",
          ...
          "required": ["@type", "trStatus", "requestDate", "actionDate"]
        }
      ]
    }
  }
...
{
  "$defs": {
    "expirationDeletionMessage": {
      "allOf": [
        { "$ref": "#/$defs/baseMessage" },
        {
          "type": "object",
          "properties": {
            "@type":        { "type": "string", "const": "expirationDeletionMessage", "readOnly": true },
            "deletionDate": { "type": "string", "format": "date-time", "readOnly": true }
          },
          "required": ["@type", "deletionDate"]
        }
      ]
    }
  }
    ...
    "autoRenewalMessage": {
      "allOf": [
        { "$ref": "#/$defs/baseMessage" },
        {
          "type": "object",
          "properties": {
            "@type":       { "type": "string", "const": "autoRenewalMessage", "readOnly": true },
            "renewalDate": { "type": "string", "format": "date-time", "readOnly": true },
            "expiryDate":  { "type": "string", "format": "date-time", "readOnly": true }
          },
          "required": ["@type", "renewalDate", "expiryDate"]
        }
      ]
    }
  }
...
The Service Notice Message Object does not extend the Base Message Object, since it is not associated with a specific Data Object.
...
```json
{
  "$defs": {
    "serviceNoticeMessage": {
      "type": "object",
      "properties": {
        "@type":   { "type": "string", "const": "serviceNoticeMessage", "readOnly": true },
        "message": { "type": "string", "readOnly": true }
      },
...
The Maintenance Message Object does not extend the Base Message Object, since it is not associated with a specific Data Object.
...
```json
{
  "$defs": {
    "maintenanceMessage": {
      "type": "object",
      "properties": {
        "@type": { "type": "string", "const": "maintenanceMessage", "readOnly": true },
...
- `type` MUST be a value registered in the IANA RPP Message Type Values Registry.
- `data` MUST contain an instance of the message object type mapped to the registered `type` value, as defined in [@!I-D.ietf-rpp-data-objects]: `review-response` maps to Review Response Object, `transfer-request` maps to Transfer Request Message Object, `transfer-outcome` maps to Transfer Outcome Message Object, `expiration-deletion` maps to Expiration Deletion Message Object, `auto-renewal` maps to Auto-Renewal Message Object, `service-notice` maps to Service Notice Message Object, and `maintenance` maps to Maintenance Message Object.
  ...
  "$defs": {
    "messageObject.create": {
      ...
      "properties": {
        ...
        "owner": { "$ref": "#/$defs/organisationObject.reference", "writeOnly": true },
        "type":  { "type": "string", "writeOnly": true, "enum": ["review-response", "transfer-request", "transfer-outcome", "expiration-deletion", "auto-renewal", "service-notice", "maintenance"] },
        "data":  { "type": "object", "writeOnly": true }
      },
      "required": ["@type", "owner", "type", "data"],
      "allOf": [
        {
          "if":   { "properties": { "type": { "const": "review-response" } }, "required": ["type"] },
          "then": { "properties": { "data": { "$ref": "#/$defs/reviewResponse" } } }
        },
        {
          "if":   { "properties": { "type": { "const": "transfer-request" } }, "required": ["type"] },
          "then": { "properties": { "data": { "$ref": "#/$defs/transferRequest" } } }
        },
        {
          "if":   { "properties": { "type": { "const": "transfer-outcome" } }, "required": ["type"] },
          "then": { "properties": { "data": { "$ref": "#/$defs/transferOutcome" } } }
        },
        {
          "if":   { "properties": { "type": { "const": "expiration-deletion" } }, "required": ["type"] },
          "then": { "properties": { "data": { "$ref": "#/$defs/expirationDeletionMessage" } } }
        },
        {
          "if":   { "properties": { "type": { "const": "auto-renewal" } }, "required": ["type"] },
          "then": { "properties": { "data": { "$ref": "#/$defs/autoRenewalMessage" } } }
        },
        {
          "if":   { "properties": { "type": { "const": "service-notice" } }, "required": ["type"] },
          "then": { "properties": { "data": { "$ref": "#/$defs/serviceNoticeMessage" } } }
        },
        {
          "if":   { "properties": { "type": { "const": "maintenance" } }, "required": ["type"] },
          "then": { "properties": { "data": { "$ref": "#/$defs/maintenanceMessage" } } }
        }
      ]
    }
  }
...
Read response schema. Note that `owner` MUST NOT be provided in the response, since a client can only ever retrieve messages linked to its own organisation:
  ...
  "$defs": {
    "messageObject.read": {
      ...
      "properties": {
        ...
        "status":       { "type": "string", "enum": ["queued", "delivered", "removed"], "readOnly": true },
        "type":         { "type": "string", "readOnly": true, "enum": ["review-response", "transfer-request", "transfer-outcome", "expiration-deletion", "auto-renewal", "service-notice", "maintenance"] },
        "data":         { "type": "object", "readOnly": true }
      },
      "required": ["@type", "id", "creationDate", "status", "type", "data"],
      "allOf": [
        {
          "if":   { "properties": { "type": { "const": "review-response" } }, "required": ["type"] },
          "then": { "properties": { "data": { "$ref": "#/$defs/reviewResponse" } } }
...
The Query operation retrieves a collection of Message Data Object instances. The `count` and `type` selectors MUST be conveyed as HTTP query string parameters, not in a JSON request body; there is no Query request schema. As with the Read operation, `owner` MUST NOT be provided in the response.
...
Example message using the `transfer-request` type, notifying the recipient organisation that a transfer of the domain name `transfer-test.example` is pending and requires action:
...
```json
{
    ...
    "data": {
        ...
        "objectType": "domain",
        "objectId": "transfer-test.example",
        "status": "pending",
        "requestDate": "2026-09-17T09:00:00.0Z"
    }
}
```
...
Example message query request for the two most recent queued messages, conveyed as HTTP query string parameters (`count=2`); there is no request body.

Example message query response, containing a `transfer-request` message for the domain `transfer-test.example` and an `expiration-deletion` message for the domain `expired.example`. Note that `owner` is not included in the response:
...
[
    ...
    },
    {
        "@type": "message",
        "id": "MSG-98766",
        "creationDate": "2026-09-18T03:15:00.0Z",
        "status": "queued",
        "type": "expiration-deletion",
        "data": {
            "@type": "expirationDeletionMessage",
            "objectType": "domain",
            "objectId": "expired.example",
            "deletionDate": "2026-09-18T03:15:00.0Z"
        }
    }
]
```
